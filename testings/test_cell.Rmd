...
# path <- "../data/time_series_image_samples2/"
path <- "../data/time_series_image_samples3/"
...
arguments <- rcell2::cellArgs2(path = path, 
                               parameters = parameters#, file.pattern = "^(BF|[A-Z]FP)_Position(\\d+)().tif$"
                               )
...
rcell2::cell2(arguments = arguments, 
              ...
              dry = T, no_cores = 1, encode_cellID_in_pixels = F, fill_interior_pixels = T)
```
...
rcell2::cell2(arguments = arguments, 
              ...
              dry = F, no_cores = 1, encode_cellID_in_pixels = T, fill_interior_pixels = T, ignore.stdout = F)
```
...
```{r}
cdata <- cell_data.new$data
paths <- cell_data.new$images
pdata <- read_csv("../data/time_series_image_samples/pdata.csv")
p <- ggplot(mapping = aes(x=t.frame, y=a.tot, group=ucid)) + 
  geom_line()

tagCell(cdata = cdata, paths = paths, pdata = pdata, 
        cell_tags = list(hola = c("asd", "asd2")), 
